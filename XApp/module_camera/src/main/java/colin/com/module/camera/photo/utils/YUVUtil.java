...
public class YUVUtil {
     ...
     * @return 默认是竖屏 旋转图片的
     */
    public static Bitmap getBitmapFromNV21(int id, int angle, byte[] buffer, int width, int height) {
        int[] i = new int[buffer.length];
        decodeYUV420SP(i, buffer, width, height);
        Bitmap bitmap = Bitmap.createBitmap(i, width, height,
                Bitmap.Config.RGB_565);
        return rotaingImageView(id, angle, bitmap, false);
    }

    public static Bitmap getBitmapFromNV21(int id, int angle, byte[] buffer, int width, int height, boolean isLandsca) {
        bufferInt = new int[buffer.length];
        decodeYUV420SP(bufferInt, buffer, width, height);
        Bitmap bitmap = Bitmap.createBitmap(bufferInt, width, height,
                Bitmap.Config.RGB_565);
        return rotaingImageView(id, angle, bitmap, isLandsca);
    }
     ...
     * @return bitmap 图片
     */
    public static Bitmap rotaingImageView(int id, int angle, Bitmap bitmap, boolean isLand) {
     ...
     *
     * @param nv21   输入的图片数据，Android读图特殊格式
     * @param width  输入图的宽度
     * @param height 输入图的高度
     * @return ARGB彩图
     */
